...
var y{D,A} binary;
## objective function
 minimize cost: sum{d in D} devcost[d] * sum{v in V} x[d,v];
## constraints
...
subject to dt5{v in V}:
  sum{d in D} x[d,v]<=1;
# devA: every node adjacent to installed node is covered
subject to dt6{u in V, v in N[u]}:
  y[member(1,D),u,v]==x[member(1,D),u];
...
subject to dt10{d in D1, u in V}:
  sum{v in N[u]} y[d,u,v]==x[d,u];
# line is covered
subject to dt12{(u,v) in E}:
